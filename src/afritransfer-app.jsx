...
const Logo = ({h=40, dark=false}) => (
  <img src={"data:image/png;base64,"+(dark ? LOGO_BLACK_B64 : LOGO_WHITE_B64)} style={{height:h, width:h*2.94, objectFit:"contain"}} alt="Cash Money"/>
);
...
const CONTACTS = [
  {id:1, nom:"Aminata Diallo",   tel:"+221 77 123 45 67", pays:"SN", op:"Wave"},
  {id:2, nom:"Hervé Mbemba",     tel:"+242 06 789 01 23", pays:"CG", op:"Airtel"},
  {id:3, nom:"Grace Lukeba",     tel:"+243 81 234 56 78", pays:"CD", op:"Orange"},
  {id:4, nom:"Fatoumata Touré",  tel:"+223 65 345 67 89", pays:"ML", op:"Wave"},
  {id:5, nom:"Mariam Ouédraogo", tel:"+226 70 456 78 90", pays:"BF", op:"Orange"},
  {id:6, nom:"Kofi Mensah",      tel:"+242 05 111 22 33", pays:"CG", op:"MTN"},
  {id:7, nom:"Paul Biya Jr",     tel:"+237 67 890 12 34", pays:"CM", op:"MTN"},
];

const HISTO0 = [
  {id:1, dest:"Aminata Diallo", flag:"\u{1F1F8}\u{1F1F3}", mnt:98250,  recu:98250,  dev:"XOF", op:"Wave",   date:"06/03/2026", ok:true},
  {id:2, dest:"Hervé Mbemba",   flag:"\u{1F1E8}\u{1F1EC}", mnt:131000, recu:131000, dev:"XAF", op:"Airtel", date:"28/02/2026", ok:false},
  {id:3, dest:"Grace Lukeba",   flag:"\u{1F1E8}\u{1F1E9}", mnt:140000, recu:140000, dev:"CDF", op:"Orange", date:"20/02/2026", ok:true},
];

const NOTIFS = [
  {id:1, ic:"✅", titre:"Transfert confirmé", msg:"98 250 FCFA envoyés à Aminata Diallo", heure:"Il y a 2h", lu:false},
  {id:2, ic:"🔔", titre:"Nouveau taux disponible", msg:"Le taux EUR/XOF a été mis à jour", heure:"Il y a 5h", lu:false},
  {id:3, ic:"🛡️", titre:"Vérification KYC", msg:"Complétez votre KYC pour débloquer +3M FCFA/mois", heure:"Hier", lu:true},
  {id:4, ic:"💸", titre:"Transfert reçu", msg:"Vous avez reçu 50 000 FCFA de Jean Dupont", heure:"Il y a 3 jours", lu:true},
];

const OP_SOLDES = { MTN: 820000, Airtel: 345000, Orange: 128000, Wave: 512500 };

const fee     = (m) => Math.round(m * 0.05);
...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .btn-press:active { transform: scale(0.96) !important; transition: transform .1s !important; }
  ::-webkit-scrollbar { width: 0; height: 0; }
  input, button { font-family: 'Plus Jakarta Sans', system-ui, sans-serif; }
`;
...
function Splash({onDone}) {
  ...
  return (
    <div style={{minHeight:"100vh", background:"linear-gradient(160deg,#0d3300,#2d6b00 50%,#6DC21E)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:32}}>
      <div style={{animation:"pulse 2s ease infinite"}}>
        <Logo h={64}/>
      </div>
      <div style={{color:"rgba(255,255,255,.7)", fontSize:14, letterSpacing:1, fontWeight:600}}>TRANSFERTS VERS L'AFRIQUE</div>
      <div style={{width:200, height:4, borderRadius:2, background:"rgba(255,255,255,.2)"}}>
        <div style={{width:`${progress}%`, height:"100%", borderRadius:2, background:"#fff", transition:"width .05s linear"}}/>
      </div>
    </div>
  );
}
...
// ── PIN ───────────────────────────────────────────────────────────────────────
function Pin({title, sub, onOk, onCancel}) {
  const [p, setP] = useState("");
  const [loading, setL] = useState(false);
  const press = (v) => {
    if (p.length >= 4 || loading) return;
    const n = p + v; setP(n);
    if (n.length === 4) { setL(true); setTimeout(() => { setL(false); onOk(); }, 700); }
  };
  const del = () => setP(x => x.slice(0,-1));
  return (
    <div className="fadeIn" style={{minHeight:"100vh", background:"linear-gradient(160deg,#0d3300,#2d6b00 50%,#6DC21E)", display:"flex", flexDirection:"column"}}>
      <div style={{padding:"50px 20px 20px", textAlign:"center"}}><Logo h={50}/></div>
      <div style={{background:"#fff", borderRadius:"28px 28px 0 0", padding:"28px 20px 40px", marginTop:"auto"}}>
        <div style={{textAlign:"center", marginBottom:24}}>
          <div style={{fontSize:19, fontWeight:800, marginBottom:6}}>{title}</div>
          <div style={{fontSize:13, color:"#8FAD72", fontWeight:500}}>{sub}</div>
        </div>
        <div style={{display:"flex", justifyContent:"center", gap:16, marginBottom:28}}>
          {[0,1,2,3].map(i => (
            <div key={i} style={{width:16, height:16, borderRadius:"50%", border:"2px solid "+(i<p.length?G:"#D8EBC0"), background:i<p.length?G:"transparent", transition:"all .15s"}}/>
          ))}
        </div>
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, maxWidth:260, margin:"0 auto"}}>
          {["1","2","3","4","5","6","7","8","9","","0","x"].map((k,i) => (
            k === "" ? <div key={i}/> :
            <button key={i} className="btn-press" onClick={() => k === "x" ? del() : press(k)}
              style={{height:58, borderRadius:14, border:"none", background:"#F7FAF2", fontSize:k==="x"?18:22, fontWeight:700, color:"#1A2E0A", cursor:"pointer"}}>
              {loading ? "" : k === "x" ? "\u232B" : k}
            </button>
          ))}
        </div>
        {loading && <div style={{textAlign:"center", marginTop:16, color:G, fontWeight:700, fontSize:13}}>Vérification...</div>}
        {onCancel && <button onClick={onCancel} style={{display:"block", margin:"20px auto 0", background:"none", border:"none", color:"#8FAD72", fontSize:13, fontWeight:600, cursor:"pointer"}}>Annuler</button>}
      </div>
    </div>
  );
}
...
function Auth({onAuth}) {
  ...
  const [paysA, setPaysA] = useState(null);
  const [showP, setShowP] = useState(false);
  const [tel, setTel]     = useState("");
  const [step, setStep]   = useState("form");
  const fullTel = paysA ? paysA.ind + " " + tel : tel;
  const ok = paysA && tel.length >= 6 && (tab === "login" || nom.length >= 2);
  if (step === "pin") return <Pin title="Bienvenue !" sub="Entrez votre code PIN (démo: 1234)" onOk={() => onAuth({nom: nom || "Utilisateur", tel: fullTel})} onCancel={() => setStep("form")}/>;
  return (
    <div className="fadeIn" style={{minHeight:"100vh", background:"linear-gradient(160deg,#0d3300,#2d6b00 50%,#6DC21E)", display:"flex", flexDirection:"column"}}>
      <div style={{padding:"52px 20px 24px", display:"flex", flexDirection:"column", alignItems:"center", gap:12}}>
        <Logo h={54}/>
        <p style={{color:"rgba(255,255,255,.8)", fontSize:13, fontWeight:500}}>Transferts d'argent vers l'Afrique</p>
      </div>
      <div style={{background:"#fff", borderRadius:"28px 28px 0 0", padding:"28px 20px 40px", marginTop:"auto"}}>
        <div style={{display:"flex", background:"#F3F7EE", borderRadius:12, padding:4, marginBottom:22}}>
          {["login","signup"].map(t => (
            <button key={t} onClick={() => setTab(t)} style={{flex:1, padding:10, border:"none", borderRadius:10, background:tab===t?"#fff":"transparent", color:tab===t?G:"#8FAD72", fontSize:14, fontWeight:800, cursor:"pointer", transition:"all .2s"}}>
              {t === "login" ? "Connexion" : "Inscription"}
            </button>
          ))}
        </div>
        {tab === "signup" && (
          <div style={{marginBottom:14}} className="fadeIn">
            <label style={{fontSize:11, fontWeight:800, color:"#4A5E38", marginBottom:5, display:"block", letterSpacing:.5}}>NOM COMPLET</label>
            <input style={{width:"100%", padding:"13px 14px", borderRadius:12, border:"1.5px solid #D8EBC0", background:"#F7FAF2", fontSize:15, color:"#1A2E0A", outline:"none"}} placeholder="Ex: Aminata Diallo" value={nom} onChange={e => setNom(e.target.value)}/>
          </div>
        )}
        <div style={{marginBottom:12}}>
          <label style={{fontSize:11, fontWeight:800, color:"#4A5E38", marginBottom:5, display:"block", letterSpacing:.5}}>PAYS</label>
          <button onClick={() => setShowP(!showP)} style={{width:"100%", padding:"13px 14px", borderRadius:12, border:"1.5px solid "+(paysA?G:"#D8EBC0"), background:"#F7FAF2", fontSize:14, color:paysA?"#1A2E0A":"#8FAD72", outline:"none", display:"flex", alignItems:"center", justifyContent:"space-between", cursor:"pointer"}}>
            <span style={{fontWeight:paysA?600:400}}>{paysA ? paysA.flag+" "+paysA.nom+" ("+paysA.ind+")" : "Sélectionnez votre pays..."}</span>
            <span style={{fontSize:11, color:"#8FAD72"}}>{showP ? "▲" : "▼"}</span>
          </button>
          {showP && (
            <div className="fadeIn" style={{background:"#fff", borderRadius:12, border:"1.5px solid #D8EBC0", overflow:"hidden", marginTop:4, boxShadow:"0 8px 24px rgba(0,0,0,.12)", position:"relative", zIndex:10}}>
              {PAYS_AUTH.map(p => (
                <div key={p.code} onClick={() => {setPaysA(p); setShowP(false);}} style={{display:"flex", alignItems:"center", gap:12, padding:"13px 14px", borderBottom:"1px solid #D8EBC0", cursor:"pointer", background:paysA?.code===p.code?"#eef8dc":"#fff"}}>
                  <span style={{fontSize:22}}>{p.flag}</span>
                  <div style={{flex:1}}>
                    <div style={{fontSize:14, fontWeight:700, color:"#1A2E0A"}}>{p.nom}</div>
                    <div style={{fontSize:12, color:"#8FAD72"}}>Indicatif : {p.ind}</div>
                  </div>
                  {paysA?.code === p.code && <Check color={G} size={16}/>}
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{marginBottom:20}}>
          <label style={{fontSize:11, fontWeight:800, color:"#4A5E38", marginBottom:5, display:"block", letterSpacing:.5}}>TÉLÉPHONE</label>
          <div style={{display:"flex", gap:8}}>
            {paysA && (
              <div style={{padding:"13px 12px", borderRadius:12, border:"1.5px solid #D8EBC0", background:"#eef8dc", fontSize:14, fontWeight:700, color:GD, whiteSpace:"nowrap", flexShrink:0}}>
                {paysA.flag} {paysA.ind}
              </div>
            )}
            <input style={{flex:1, padding:"13px 14px", borderRadius:12, border:"1.5px solid #D8EBC0", background:paysA?"#F7FAF2":"#f0f0f0", fontSize:15, color:"#1A2E0A", outline:"none"}}
              type="tel" placeholder={paysA ? "06 12 34 56 78" : "Choisir un pays d'abord"} disabled={!paysA} value={tel} onChange={e => setTel(e.target.value.replace(/[^0-9 ]/g, ""))}/>
          </div>
        </div>
        <button className="btn-press" style={{width:"100%", padding:15, border:"none", borderRadius:14, background:ok?"linear-gradient(135deg,"+G+","+GD+")":"#D8EBC0", color:ok?"#fff":"#8FAD72", fontSize:15, fontWeight:800, cursor:ok?"pointer":"not-allowed", transition:"all .2s"}} disabled={!ok} onClick={() => setStep("pin")}>
          <span style={{display:"flex", alignItems:"center", justifyContent:"center", gap:8}}>
            {tab === "login" ? "Se connecter" : "Créer mon compte"}<ChevR/>
          </span>
        </button>
      </div>
    </div>
...
function generateQRPattern(text) {
  ...
  for (let r = 0; r < 21; r++) {
    for (let c = 0; c < 21; c++) {
      ...
      if (inFinder) {
        const fr = r < 8 ? r : r - 14, fc = c < 8 ? c : c - 14;
        const inBox = fr >= 0 && fr <= 6 && fc >= 0 && fc <= 6;
        const onEdge = inBox && (fr === 0 || fr === 6 || fc === 0 || fc === 6);
        const inner = inBox && fr >= 2 && fr <= 4 && fc >= 2 && fc <= 4;
        cells.push(onEdge || inner ? 1 : 0);
      } else {
        const seed = (hash ^ (r * 137 + c * 149 + r * c * 23)) & 0xFFFF;
...
function QRCode({text, size=160, color="#000", bg="#fff"}) {
  const cells = generateQRPattern(text), n = 21, cell = size / n;
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} xmlns="http://www.w3.org/2000/svg">
      ...
      {cells.map((v, i) => {
        ...
        return <rect key={i} x={c*cell} y={r*cell} width={cell} height={cell} fill={color}/>;
      })}
    </svg>
  );
}
...
// ── QR MODAL ──────────────────────────────────────────────────────────────────
function QRModal({op, user, onClose}) {
  const [step, setStep] = useState("qr");
  const [pwd, setPwd]   = useState("");
  const [error, setErr] = useState(false);
  const qrData = `cashmoney://receive?op=${op}&user=${encodeURIComponent(user?.tel||"")}&t=${Date.now()}`;
  const press = (v) => {
    if (pwd.length >= 4) return;
    const n = pwd + v; setPwd(n);
    if (n.length === 4) {
      if (n === "1234") { setStep("success"); }
      else { setErr(true); setTimeout(() => { setErr(false); setPwd(""); }, 1000); }
    }
  };
  return (
    <div style={{position:"fixed", inset:0, background:"rgba(0,0,0,.6)", zIndex:200, display:"flex", alignItems:"flex-end", justifyContent:"center"}}>
      <div className="slideUp" style={{background:"#fff", borderRadius:"24px 24px 0 0", width:"100%", maxWidth:430, padding:"24px 20px 40px"}}>
        {step === "qr" && (
          <>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18}}>
              <div style={{fontSize:17, fontWeight:800}}>Mon QR Code {op}</div>
              <button onClick={onClose} style={{background:"none", border:"none", fontSize:22, cursor:"pointer", color:"#8FAD72"}}>✕</button>
            </div>
            <div style={{display:"flex", flexDirection:"column", alignItems:"center", gap:16}}>
              <div style={{background:"#F7FAF2", borderRadius:16, padding:16}}>
                <div style={{borderRadius:12, overflow:"hidden", border:"3px solid "+G}}>
                  <QRCode text={qrData} size={180} color="#1A2E0A" bg="#fff"/>
                </div>
              </div>
              <div style={{display:"flex", alignItems:"center", gap:10}}>
                <OpImg name={op} size={32}/>
                <div>
                  <div style={{fontSize:13, fontWeight:700}}>{op} Mobile Money</div>
                  <div style={{fontSize:11, color:"#8FAD72"}}>{user?.tel}</div>
                </div>
              </div>
              <div style={{background:"#eef8dc", borderRadius:10, padding:"10px 14px", fontSize:12, color:GD, textAlign:"center", lineHeight:1.5}}>
                Faites scanner ce QR code pour recevoir de l'argent instantanément.
              </div>
              <button className="btn-press" onClick={() => setStep("pin")} style={{width:"100%", padding:14, border:"none", borderRadius:12, background:"linear-gradient(135deg,#6DC21E,#3d7a00)", color:"#fff", fontSize:15, fontWeight:800, cursor:"pointer"}}>
                Valider par PIN
              </button>
            </div>
          </>
        ...
        {step === "pin" && (
          <>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18}}>
              <div style={{fontSize:17, fontWeight:800}}>Entrez votre PIN</div>
              <button onClick={() => setStep("qr")} style={{background:"none", border:"none", fontSize:14, cursor:"pointer", color:"#8FAD72", fontWeight:600}}>Retour</button>
            </div>
            <div style={{textAlign:"center", marginBottom:20}}>
              <OpImg name={op} size={44} style={{margin:"0 auto 8px"}}/>
              <div style={{fontSize:13, color:"#8FAD72"}}>Validez votre identité pour autoriser la réception</div>
            </div>
            <div style={{display:"flex", justifyContent:"center", gap:14, marginBottom:24}}>
              {[0,1,2,3].map(i => (
                <div key={i} style={{width:16, height:16, borderRadius:"50%", border:"2px solid "+(error?"#e53e3e":i<pwd.length?G:"#D8EBC0"), background:error?"#e53e3e":i<pwd.length?G:"transparent", transition:"all .15s"}}/>
              ))}
            </div>
            <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, maxWidth:260, margin:"0 auto"}}>
              {["1","2","3","4","5","6","7","8","9","","0","x"].map((k,i) => (
                k === "" ? <div key={i}/> :
                <button key={i} className="btn-press" onClick={() => k==="x" ? setPwd(p=>p.slice(0,-1)) : press(k)}
                  style={{height:54, borderRadius:12, border:"none", background:"#F7FAF2", fontSize:k==="x"?18:22, fontWeight:700, color:error?"#e53e3e":"#1A2E0A", cursor:"pointer"}}>
                  {k==="x"?"\u232B":k}
                </button>
              ))}
            </div>
            {error && <div style={{textAlign:"center", marginTop:12, color:"#e53e3e", fontSize:13, fontWeight:700}}>PIN incorrect. Réessayez.</div>}
          </>
        )}
        {step === "success" && (
          <div className="fadeIn" style={{textAlign:"center", padding:"10px 0"}}>
            <div style={{fontSize:52, marginBottom:12}}>✅</div>
            <div style={{fontSize:20, fontWeight:800, marginBottom:6}}>Identité vérifiée !</div>
            <div style={{fontSize:14, color:"#8FAD72", marginBottom:20}}>Votre QR Code {op} est actif et prêt.</div>
            <div style={{background:"#F7FAF2", borderRadius:12, padding:"12px 16px", marginBottom:20, display:"inline-block"}}>
              <QRCode text={qrData} size={140} color="#1A2E0A" bg="#F7FAF2"/>
            </div>
            <button className="btn-press" onClick={onClose} style={{width:"100%", padding:14, border:"none", borderRadius:12, background:"linear-gradient(135deg,#6DC21E,#3d7a00)", color:"#fff", fontSize:15, fontWeight:800, cursor:"pointer"}}>
              Fermer
            </button>
          </div>
        )}
...
function TxRow({t}) {
  return (
    <div style={{display:"flex", alignItems:"center", gap:12, background:"#fff", borderRadius:16, padding:14, marginBottom:8}}>
      <div style={{width:46, height:46, borderRadius:14, background:"#f0f9e8", display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, flexShrink:0}}>{t.flag}</div>
      <div style={{flex:1, minWidth:0}}>
        <div style={{fontSize:14, fontWeight:700, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap"}}>{t.dest}</div>
        <div style={{display:"flex", alignItems:"center", gap:6, marginTop:3}}>
          <OpImg name={t.op} size={16}/>
          <span style={{fontSize:11, color:"#8FAD72", fontWeight:500}}>{t.op} · {t.date}</span>
        </div>
        <span style={{display:"inline-block", padding:"2px 8px", borderRadius:20, fontSize:10, fontWeight:700, marginTop:4, background:t.ok?"#e0f9ef":"#fff4e0", color:t.ok?"#00956b":"#cc8800"}}>
          {t.ok ? "Envoyé" : "En attente"}
        </span>
      </div>
      <div style={{textAlign:"right", flexShrink:0}}>
        <div style={{fontSize:14, fontWeight:800}}>-{t.mnt.toLocaleString("fr-FR")} F</div>
        {t.dev !== "XOF" && <div style={{fontSize:10, color:"#8FAD72", marginTop:2, fontWeight:500}}>{t.recu.toLocaleString("fr-FR")} {t.dev}</div>}
      </div>
    </div>
  );
}
...
function Notifications({notifs, onBack, onMarkAll}) {
  return (
    <div className="fadeIn" style={{flex:1, overflowY:"auto", paddingBottom:85, background:"#F3F7EE"}}>
      <div style={{padding:"14px 16px 10px", background:"#fff", borderBottom:"1px solid #D8EBC0", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:10}}>
        <button onClick={onBack} style={{display:"flex", alignItems:"center", gap:6, background:"none", border:"none", cursor:"pointer", fontSize:15, fontWeight:700, color:"#1A2E0A"}}>
          <ChevL/>Retour
        </button>
        <div style={{fontSize:17, fontWeight:800}}>Notifications</div>
        <button onClick={onMarkAll} style={{background:"none", border:"none", fontSize:12, fontWeight:700, color:G, cursor:"pointer"}}>Tout lire</button>
      </div>
      <div style={{padding:"12px 14px 0"}}>
        {notifs.map((n,i) => (
          <div key={n.id} className="fadeIn" style={{display:"flex", gap:12, background:"#fff", borderRadius:16, padding:14, marginBottom:8, borderLeft:"3px solid "+(n.lu?"transparent":G), animationDelay:`${i*0.05}s`}}>
            <div style={{width:44, height:44, borderRadius:14, background:n.lu?"#F7FAF2":"#eef8dc", display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, flexShrink:0}}>{n.ic}</div>
            <div style={{flex:1}}>
              <div style={{fontSize:14, fontWeight:n.lu?600:800, color:"#1A2E0A"}}>{n.titre}</div>
              <div style={{fontSize:12, color:"#8FAD72", marginTop:3, lineHeight:1.4, fontWeight:500}}>{n.msg}</div>
              <div style={{fontSize:11, color:"#B0C4A0", marginTop:5, fontWeight:500}}>{n.heure}</div>
            </div>
            {!n.lu && <div style={{width:8, height:8, borderRadius:"50%", background:G, flexShrink:0, marginTop:4}}/>}
          </div>
        ))}
      </div>
    </div>
...
// ── HOME ──────────────────────────────────────────────────────────────────────
function Home({solde, histo, onSend, onHisto, user, onNotif, notifCount}) {
  const [selOp, setSelOp]       = useState("MTN");
  const [showSolde, setShowSolde] = useState(false);
  const [activeQR, setActiveQR] = useState(null); // op dont le QR est affiché
  const soldeOp = OP_SOLDES[selOp] || solde;

  // QR data pour chaque opérateur
  const qrFor = (op) => `cashmoney://deposit?op=${op}&user=${encodeURIComponent(user?.tel||"")}&t=${Date.now()}`;

  return (
    <div className="fadeIn" style={{flex:1, overflowY:"auto", paddingBottom:85}}>
      <style>{css}</style>
      ...
      {activeQR && (
        <div style={{position:"fixed", inset:0, background:"rgba(0,0,0,.65)", zIndex:200, display:"flex", alignItems:"flex-end", justifyContent:"center"}}>
          <div className="slideUp" style={{background:"#fff", borderRadius:"24px 24px 0 0", width:"100%", maxWidth:430, padding:"24px 20px 44px"}}>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20}}>
              <div style={{display:"flex", alignItems:"center", gap:10}}>
                <OpImg name={activeQR} size={36}/>
                <div>
                  <div style={{fontSize:16, fontWeight:800}}>{activeQR} Mobile Money</div>
                  <div style={{fontSize:12, color:"#8FAD72", fontWeight:500}}>QR Code de dépôt</div>
                </div>
              </div>
              <button onClick={() => setActiveQR(null)} style={{background:"none", border:"none", fontSize:24, cursor:"pointer", color:"#8FAD72", lineHeight:1}}>✕</button>
            </div>
            <div style={{display:"flex", flexDirection:"column", alignItems:"center", gap:14}}>
              <div style={{background:"#F7FAF2", borderRadius:18, padding:18}}>
                <div style={{borderRadius:14, overflow:"hidden", border:"3px solid "+G}}>
                  <QRCode text={qrFor(activeQR)} size={200} color="#1A2E0A" bg="#fff"/>
                </div>
              </div>
              <div style={{textAlign:"center"}}>
                <div style={{fontSize:14, fontWeight:700, color:"#1A2E0A"}}>{user?.tel}</div>
                <div style={{fontSize:12, color:"#8FAD72", marginTop:4, fontWeight:500}}>Solde actuel : {showSolde ? OP_SOLDES[activeQR].toLocaleString("fr-FR")+" FCFA" : "••••••"}</div>
              </div>
              <div style={{background:"#eef8dc", borderRadius:12, padding:"11px 16px", fontSize:12, color:GD, textAlign:"center", lineHeight:1.6, fontWeight:500}}>
                📲 Faites scanner ce QR code par l'expéditeur pour recevoir un dépôt {activeQR} instantanément.
              </div>
              <button className="btn-press" onClick={() => setActiveQR(null)} style={{width:"100%", padding:14, border:"none", borderRadius:14, background:"linear-gradient(135deg,"+G+","+GD+")", color:"#fff", fontSize:15, fontWeight:800, cursor:"pointer"}}>
                Fermer
              </button>
            </div>
          </div>
      ...
      <div style={{padding:"14px 16px 10px", background:"#fff", borderBottom:"1px solid #D8EBC0", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:10}}>
        ...
        <button onClick={onNotif} style={{position:"relative", background:"none", border:"none", cursor:"pointer", padding:4}}>
          ...
          {notifCount > 0 && (
            <span style={{position:"absolute", top:2, right:2, width:16, height:16, borderRadius:"50%", background:"#e53e3e", color:"#fff", fontSize:9, fontWeight:800, display:"flex", alignItems:"center", justifyContent:"center"}}>{notifCount}</span>
          )}
        </button>
      </div>

      {/* Carte solde */}
      <div style={{margin:"14px 14px 0", background:"linear-gradient(135deg,#0d3300,#3d7a00 60%,#6DC21E)", borderRadius:22, padding:"20px 18px", color:"#fff"}}>
        {/* Solde avec œil */}
        <div style={{display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:2}}>
          <div style={{fontSize:12, opacity:.75, fontWeight:600, letterSpacing:.5}}>SOLDE DISPONIBLE</div>
          <button onClick={() => setShowSolde(v => !v)} style={{background:"none", border:"none", cursor:"pointer", padding:"2px 4px", color:"rgba(255,255,255,.8)", fontSize:18, lineHeight:1}}>
            {showSolde ? "👁️" : "🙈"}
          </button>
        </div>
        <div style={{fontSize:38, fontWeight:800, letterSpacing:-1, transition:"all .3s"}}>
          {showSolde ? soldeOp.toLocaleString("fr-FR") : "••••••"}
        </div>
        <div style={{fontSize:14, opacity:.75, marginTop:2, fontWeight:600}}>FCFA</div>

        {/* Sélecteur opérateur avec QR inline */}
        <div style={{marginTop:16}}>
          <div style={{fontSize:11, opacity:.75, fontWeight:600, letterSpacing:.5, marginBottom:8}}>MES PORTEFEUILLES</div>
          <div style={{display:"flex", gap:8}}>
            {ALL_OPS.map(op => (
              <button key={op} onClick={() => setSelOp(selOp === op ? selOp : op)} className="btn-press"
                style={{flex:1, background:selOp===op?"rgba(255,255,255,.22)":"rgba(255,255,255,.07)", border:"1.5px solid "+(selOp===op?"rgba(255,255,255,.5)":"rgba(255,255,255,.12)"), borderRadius:12, padding:"8px 4px", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:4, transition:"all .2s"}}>
                <OpImg name={op} size={26}/>
                <span style={{fontSize:8, color:"#fff", fontWeight:700, opacity:selOp===op?1:.65}}>{op}</span>
                {/* Mini QR icon */}
                <button onClick={(e) => { e.stopPropagation(); setSelOp(op); setActiveQR(op); }} style={{background:"rgba(255,255,255,.2)", border:"none", borderRadius:6, padding:"3px 5px", cursor:"pointer", fontSize:9, color:"#fff", fontWeight:700, marginTop:1}}>
                  📱 QR
                </button>
              </button>
            ))}
        ...
        <div style={{display:"flex", gap:8, marginTop:12}}>
          <button className="btn-press" onClick={onSend} style={{flex:1, padding:11, borderRadius:12, background:"#fff", border:"none", color:GD, fontSize:13, fontWeight:800, cursor:"pointer"}}>
            ✈️ Envoyer
          </button>
          <button className="btn-press" onClick={onHisto} style={{flex:2, padding:11, borderRadius:12, background:"rgba(255,255,255,.15)", border:"1px solid rgba(255,255,255,.25)", color:"#fff", fontSize:13, fontWeight:700, cursor:"pointer"}}>
            🕐 Historique
          </button>
        </div>
      </div>
      ...
      {/* KYC Banner */}
      <div style={{margin:"12px 14px 0", background:"linear-gradient(135deg,#F5A623,#E8901A)", borderRadius:16, padding:"13px 15px", display:"flex", alignItems:"center", gap:10, color:"#fff", cursor:"pointer"}}>
        <span style={{fontSize:20}}>🛡️</span>
        <div style={{flex:1}}>
          <div style={{fontSize:13, fontWeight:800}}>Vérification KYC</div>
          <div style={{fontSize:11, opacity:.9, marginTop:1, fontWeight:500}}>Augmentez votre limite à 3 000 000 FCFA/mois</div>
        </div>
        <ChevR/>
      </div>
      ...
      <div style={{padding:"18px 14px 0"}}>
        <div style={{fontSize:16, fontWeight:800, marginBottom:12}}>Envoi rapide</div>
        <div style={{display:"flex", gap:10, overflowX:"auto", paddingBottom:8}}>
          {CONTACTS.map(c => {
            const p = getPays(c.pays);
            if (!p) return null;
            return (
              <div key={c.id} onClick={() => onSend(c)} className="btn-press" style={{flexShrink:0, background:"#fff", borderRadius:16, padding:"11px 12px", cursor:"pointer", border:"1.5px solid #D8EBC0", textAlign:"center", minWidth:74}}>
                <div style={{width:42, height:42, borderRadius:"50%", background:avc(c.nom), display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, fontWeight:800, color:"#fff", margin:"0 auto 6px"}}>{ini(c.nom)}</div>
                <div style={{fontSize:10, fontWeight:700, color:"#4A5E38"}}>{c.nom.split(" ")[0]}</div>
      ...
      <div style={{padding:"18px 14px 0"}}>
        <div style={{display:"flex", justifyContent:"space-between", marginBottom:12}}>
          <div style={{fontSize:16, fontWeight:800}}>Transactions récentes</div>
          <button onClick={onHisto} style={{fontSize:12, color:G, fontWeight:700, background:"none", border:"none", cursor:"pointer"}}>Voir tout</button>
        </div>
        {histo.slice(0,3).map(t => <TxRow key={t.id} t={t}/>)}
      </div>

      {/* Taux du jour */}
      <div style={{padding:"14px 14px 8px"}}>
        <div style={{background:"#fff", borderRadius:16, padding:"14px 15px"}}>
          <div style={{fontSize:14, fontWeight:800, marginBottom:12}}>💱 Taux du jour</div>
          {PAYS.map(p => (
            <div key={p.code} style={{display:"flex", justifyContent:"space-between", fontSize:12, color:"#4A5E38", padding:"6px 0", borderBottom:"1px solid #D8EBC0"}}>
              <span style={{fontWeight:500}}>{p.flag} 1 000 FCFA</span>
              <span style={{fontWeight:800}}>{(p.taux/655*1000).toFixed(0)} {p.devise}</span>
            </div>
          ))}
        </div>
      </div>
...
// ── SEND ──────────────────────────────────────────────────────────────────────
function Send({pre, onBack, onDone}) {
  const [step, setStep]     = useState(0);
  const [src, setSrc]       = useState(null);
  const [montant, setMnt]   = useState("0");
  const [contact, setCont]  = useState(pre || null);
  const [paysD, setPaysD]   = useState(pre ? getPays(pre.pays) : null);
  const [opDest, setOpDest] = useState(pre ? pre.op : null);
  const [q, setQ]           = useState("");
  const amt = parseInt(montant) || 0, f = fee(amt), total = amt + f;
  const recu = paysD ? Math.round(amt * (paysD.taux / 655)) : 0;
  const pressNum = (v) => { if (montant==="0" && v!=="0") { setMnt(v); return; } if (montant==="0" && v==="0") return; setMnt(m => m+v); };
  const canNext = [src!==null, amt>=200, contact!==null&&paysD!==null, opDest!==null, true][step] || false;
  const goNext  = () => { if (step < 4) setStep(s => s+1); else setStep(5); };
  const goBack  = () => { if (step === 0) onBack(); else setStep(s => s-1); };
  const filtered = CONTACTS.filter(c => { const p = getPays(c.pays); return p && (c.nom.toLowerCase().includes(q.toLowerCase()) || c.tel.includes(q)); });
  const titres = ["Mon compte source","Montant à envoyer","Destinataire","Mode de transfert","Récapitulatif"];
  if (step === 5) return <Pin title="Confirmer le transfert" sub="Entrez votre PIN pour valider" onOk={() => onDone({contact, pays:paysD, op:opDest, src, amt, fee:f, total, recu})} onCancel={() => setStep(4)}/>;
  return (
    <div className="fadeIn" style={{flex:1, overflowY:"auto", paddingBottom:20, background:"#F3F7EE"}}>
      <style>{css}</style>
      <div style={{padding:"14px 16px 10px", background:"#fff", borderBottom:"1px solid #D8EBC0", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:10}}>
        <button onClick={goBack} style={{display:"flex", alignItems:"center", gap:6, background:"none", border:"none", cursor:"pointer", fontSize:15, fontWeight:700, color:"#1A2E0A"}}>
          <ChevL/>{step === 0 ? "Retour" : "Précédent"}
        </button>
        <div style={{fontSize:12, color:"#8FAD72", fontWeight:700}}>{step+1} / 5</div>
      </div>
      <div style={{padding:"12px 14px 0"}}>
        <div style={{display:"flex", gap:3, marginBottom:14}}>
          {[0,1,2,3,4].map(i => <div key={i} style={{flex:1, height:4, borderRadius:2, background:i<=step?G:"#D8EBC0", transition:"background .3s"}}/>)}
        </div>
        <div style={{fontSize:18, fontWeight:800, marginBottom:14}}>{titres[step]}</div>
      </div>
      <div style={{padding:"0 14px"}}>
        {step === 0 && (
          <div className="fadeIn">
            <div style={{background:"#eef8dc", borderRadius:12, padding:"10px 14px", marginBottom:14, fontSize:13, color:GD, fontWeight:500}}>Choisissez le compte depuis lequel l'argent sera envoyé.</div>
            <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:12}}>
              {ALL_OPS.map(o => (
                <div key={o} onClick={() => setSrc(o)} className="btn-press" style={{borderRadius:16, padding:"18px 10px", border:"2.5px solid "+(src===o?G:"#D8EBC0"), background:src===o?"#eef8dc":"#fff", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:8, transition:"all .2s"}}>
                  <OpImg name={o} size={58}/>
                  <div style={{fontSize:13, fontWeight:800}}>{o}</div>
                  <div style={{fontSize:10, color:"#8FAD72", fontWeight:500}}>Mobile Money</div>
                  <div style={{fontSize:11, fontWeight:700, color:GD}}>{OP_SOLDES[o].toLocaleString("fr-FR")} F</div>
                  {src === o && <div style={{width:22, height:22, borderRadius:"50%", background:G, display:"flex", alignItems:"center", justifyContent:"center"}}><Check color="#fff" size={10}/></div>}
                </div>
              ))}
            </div>
          </div>
        ...
        {step === 1 && (
          <div className="fadeIn" style={{background:"#fff", borderRadius:16, padding:16, marginBottom:12}}>
            <div style={{display:"flex", alignItems:"center", gap:10, background:"#F7FAF2", borderRadius:10, padding:"10px 12px", marginBottom:16}}>
              <OpImg name={src} size={36}/>
              <div>
                <div style={{fontSize:11, color:"#8FAD72", fontWeight:500}}>Compte source</div>
                <div style={{fontSize:13, fontWeight:800}}>{src} Mobile Money</div>
              </div>
            </div>
            <div style={{textAlign:"center", padding:"12px 0 16px"}}>
              <div style={{fontSize:13, color:"#8FAD72", marginBottom:6, fontWeight:500}}>Montant à envoyer</div>
              <div style={{fontSize:54, fontWeight:800, letterSpacing:-2, color:"#1A2E0A", lineHeight:1}}>{amt.toLocaleString("fr-FR")}</div>
              <div style={{fontSize:15, color:"#8FAD72", marginTop:4, fontWeight:700}}>FCFA</div>
              {amt > 0 && amt < 200 && <div style={{fontSize:12, color:"#e53e3e", marginTop:6, fontWeight:600}}>Minimum : 200 FCFA</div>}
            </div>
            <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:14}}>
              {["1","2","3","4","5","6","7","8","9","000","0","x"].map(k => (
                <button key={k} className="btn-press" onClick={() => { if (k==="x") setMnt(m => m.length<=1?"0":m.slice(0,-1)); else if (k==="000") setMnt(m => m==="0"?m:m+"000"); else pressNum(k); }}
                  style={{height:58, borderRadius:14, border:"none", background:"#F7FAF2", fontSize:k==="x"?20:22, fontWeight:700, color:"#1A2E0A", cursor:"pointer"}}>
                  {k === "x" ? "\u232B" : k}
                </button>
              ))}
            </div>
            <div style={{borderTop:"1px solid #D8EBC0", paddingTop:12}}>
              {[["Montant",amt.toLocaleString("fr-FR")+" FCFA"],["Frais (5%)",f.toLocaleString("fr-FR")+" FCFA"],["Total débité",total.toLocaleString("fr-FR")+" FCFA"]].map(([l,v]) => (
                <div key={l} style={{display:"flex", justifyContent:"space-between", fontSize:13, padding:"6px 0"}}>
                  <span style={{color:"#8FAD72", fontWeight:500}}>{l}</span>
                  <span style={{fontWeight:l.includes("Total")?800:700, color:l.includes("Total")?G:"#1A2E0A"}}>{v}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {step === 2 && (
          <div className="fadeIn">
            <div style={{position:"relative", marginBottom:12}}>
              <div style={{position:"absolute", left:11, top:12, fontSize:16}}>🔍</div>
              <input style={{width:"100%", padding:"12px 14px 12px 36px", borderRadius:12, border:"1.5px solid #D8EBC0", background:"#F7FAF2", fontSize:14, outline:"none"}} placeholder="Rechercher un contact..." value={q} onChange={e => setQ(e.target.value)}/>
            </div>
            <div style={{background:"#fff", borderRadius:16, overflow:"hidden"}}>
              {filtered.map(c => {
                const p = getPays(c.pays), sel = contact?.id === c.id;
                return (
                  <div key={c.id} onClick={() => {setCont(c); setPaysD(p); setOpDest(c.op);}} style={{display:"flex", alignItems:"center", gap:12, padding:"13px 14px", borderBottom:"1px solid #D8EBC0", cursor:"pointer", background:sel?"#eef8dc":"#fff"}}>
                    <div style={{width:46, height:46, borderRadius:"50%", background:avc(c.nom), display:"flex", alignItems:"center", justifyContent:"center", fontSize:15, fontWeight:800, color:"#fff", flexShrink:0}}>{ini(c.nom)}</div>
                    <div style={{flex:1}}>
                      <div style={{fontSize:14, fontWeight:700}}>{c.nom}</div>
                      <div style={{fontSize:12, color:"#8FAD72", fontWeight:500}}>{c.tel}</div>
                      <div style={{display:"flex", alignItems:"center", gap:6, marginTop:3}}>
                        <span style={{fontSize:14}}>{p.flag}</span>
                        <span style={{fontSize:11, color:"#8FAD72", fontWeight:500}}>{p.nom}</span>
                        <OpImg name={c.op} size={16}/>
                      </div>
                    </div>
                    {sel && <Check color={G} size={16}/>}
                  </div>
                );
              })}
            </div>
        ...
        {step === 3 && (
          <div className="fadeIn">
            {contact && (
              <div style={{background:"#eef8dc", borderRadius:12, padding:"10px 14px", marginBottom:14}}>
                <div style={{fontSize:12, color:GD, fontWeight:600}}>Destinataire : <span style={{fontWeight:800}}>{contact.nom}</span></div>
                <div style={{fontSize:11, color:"#8FAD72", marginTop:2, fontWeight:500}}>{paysD?.flag} {paysD?.nom} · {contact.tel}</div>
              </div>
            )}
            <div style={{background:"#fff", borderRadius:16, padding:16, marginBottom:12}}>
              <div style={{fontSize:14, fontWeight:800, marginBottom:12}}>Pays de destination</div>
              <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:18}}>
                {PAYS.map(p => (
                  <div key={p.code} onClick={() => {setPaysD(p); setOpDest(null);}} className="btn-press" style={{borderRadius:12, padding:"10px 6px", border:"2px solid "+(paysD?.code===p.code?G:"#D8EBC0"), background:paysD?.code===p.code?"#eef8dc":"#fff", cursor:"pointer", textAlign:"center"}}>
                    <div style={{fontSize:20}}>{p.flag}</div>
                    <div style={{fontSize:8, fontWeight:700, color:"#4A5E38", marginTop:3}}>{p.nom.split(" ")[0]}</div>
                  </div>
                ))}
              </div>
              <div style={{fontSize:14, fontWeight:800, marginBottom:12}}>Opérateur de réception</div>
              <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:12}}>
                {(paysD ? paysD.ops : ALL_OPS).map(o => (
                  <div key={o} onClick={() => setOpDest(o)} className="btn-press" style={{borderRadius:16, padding:"14px 10px", border:"2.5px solid "+(opDest===o?G:"#D8EBC0"), background:opDest===o?"#eef8dc":"#fff", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:8}}>
                    <OpImg name={o} size={48}/>
                    <div style={{fontSize:12, fontWeight:800}}>{o}</div>
                    {opDest === o && <div style={{width:20, height:20, borderRadius:"50%", background:G, display:"flex", alignItems:"center", justifyContent:"center"}}><Check color="#fff" size={9}/></div>}
                  </div>
                ))}
              </div>
            </div>
        ...
        {step === 4 && contact && paysD && (
          <div className="fadeIn" style={{background:"#fff", borderRadius:16, padding:16, marginBottom:12}}>
            <div style={{textAlign:"center", marginBottom:16}}>
              <div style={{width:58, height:58, borderRadius:"50%", background:avc(contact.nom), display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, fontWeight:800, color:"#fff", margin:"0 auto 10px"}}>{ini(contact.nom)}</div>
              <div style={{fontSize:30, fontWeight:800}}>{amt.toLocaleString("fr-FR")} FCFA</div>
              <div style={{color:"#8FAD72", fontSize:13, marginTop:2, fontWeight:500}}>vers {contact.nom}</div>
            </div>
            <div style={{height:1, background:"#D8EBC0", margin:"0 0 12px"}}/>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom:"1px dashed #D8EBC0"}}>
              <span style={{fontSize:12, color:"#8FAD72", fontWeight:500}}>Compte source</span>
              <div style={{display:"flex", alignItems:"center", gap:6}}><OpImg name={src} size={22}/><span style={{fontSize:13, fontWeight:700}}>{src}</span></div>
            </div>
            {[["Destinataire",contact.nom],["Téléphone",contact.tel],["Pays",paysD.flag+" "+paysD.nom]].map(([l,v]) => (
              <div key={l} style={{display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom:"1px dashed #D8EBC0"}}>
                <span style={{fontSize:12, color:"#8FAD72", fontWeight:500}}>{l}</span><span style={{fontSize:13, fontWeight:700}}>{v}</span>
              </div>
            ))}
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom:"1px dashed #D8EBC0"}}>
              <span style={{fontSize:12, color:"#8FAD72", fontWeight:500}}>Mode de transfert</span>
              <div style={{display:"flex", alignItems:"center", gap:6}}><OpImg name={opDest} size={22}/><span style={{fontSize:13, fontWeight:700}}>{opDest} Money</span></div>
            </div>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom:"1px dashed #D8EBC0"}}>
              <span style={{fontSize:12, color:"#8FAD72", fontWeight:500}}>Montant reçu</span>
              <span style={{fontSize:13, fontWeight:800, color:G}}>{recu.toLocaleString("fr-FR")} {paysD.devise}</span>
            </div>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 0", borderBottom:"1px dashed #D8EBC0"}}>
              <span style={{fontSize:12, color:"#8FAD72", fontWeight:500}}>Frais (5%)</span>
              <span style={{fontSize:13, fontWeight:700}}>{f.toLocaleString("fr-FR")} FCFA</span>
            </div>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", padding:"12px 0"}}>
              <span style={{fontSize:13, fontWeight:800}}>Total débité</span>
              <span style={{fontSize:15, fontWeight:800, color:G}}>{total.toLocaleString("fr-FR")} FCFA</span>
            </div>
            <div style={{background:"#eef8dc", borderRadius:10, padding:"10px 13px", display:"flex", gap:8, marginTop:4}}>
              <span>🛡️</span><span style={{fontSize:12, color:GD, lineHeight:1.5, fontWeight:500}}>Votre code PIN sera demandé pour valider ce transfert.</span>
            </div>
          </div>
        )}
      </div>
      <div style={{padding:"0 14px 14px"}}>
        <button className="btn-press" style={{width:"100%", padding:15, border:"none", borderRadius:14, background:canNext?"linear-gradient(135deg,"+G+","+GD+")":"#D8EBC0", color:canNext?"#fff":"#8FAD72", fontSize:15, fontWeight:800, cursor:canNext?"pointer":"not-allowed", transition:"all .2s"}} disabled={!canNext} onClick={goNext}>
          <span style={{display:"flex", alignItems:"center", justifyContent:"center", gap:8}}>
            {step < 4 ? "Continuer" : "Confirmer avec PIN"}<ChevR/>
          </span>
        </button>
      </div>
    </div>
...
// ── SUCCESS ───────────────────────────────────────────────────────────────────
function Success({data, nvSolde, onBack}) {
  const ref = "CMY-" + Date.now().toString().slice(-8);
  const [copied, setCopied] = useState(false);
  return (
    <div className="fadeIn" style={{flex:1, overflowY:"auto", display:"flex", flexDirection:"column", alignItems:"center", textAlign:"center", padding:"32px 20px", minHeight:"80vh", justifyContent:"center"}}>
      <style>{css}</style>
      <div style={{width:90, height:90, borderRadius:"50%", background:"linear-gradient(135deg,#eef8dc,#c8f5a0)", display:"flex", alignItems:"center", justifyContent:"center", marginBottom:22, fontSize:42}}>✅</div>
      <h2 style={{fontSize:25, fontWeight:800, margin:"0 0 8px"}}>Transfert envoyé !</h2>
      <p style={{color:"#8FAD72", fontSize:14, lineHeight:1.6, maxWidth:280, margin:"0 0 20px", fontWeight:500}}>
        {data.contact.nom} va recevoir {data.recu.toLocaleString("fr-FR")} {data.pays.devise} via {data.op}.
      </p>
      <div style={{background:"#F7FAF2", borderRadius:16, padding:16, width:"100%", marginBottom:14}}>
        <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 0", borderBottom:"1px solid #D8EBC0"}}>
          <span style={{fontSize:12, color:"#8FAD72", fontWeight:500}}>Compte source</span>
          <div style={{display:"flex", alignItems:"center", gap:6}}><OpImg name={data.src} size={20}/><span style={{fontSize:13, fontWeight:700}}>{data.src}</span></div>
        </div>
        {[["Destinataire",data.contact.nom],["Mode transfert",data.op+" Money"],["Envoyé",data.amt.toLocaleString("fr-FR")+" FCFA"],["Reçu",data.recu.toLocaleString("fr-FR")+" "+data.pays.devise],["Frais",data.fee.toLocaleString("fr-FR")+" FCFA"]].map(([l,v]) => (
          <div key={l} style={{display:"flex", justifyContent:"space-between", fontSize:13, padding:"6px 0", borderBottom:"1px solid #D8EBC0"}}>
            <span style={{color:"#8FAD72", fontWeight:500}}>{l}</span><span style={{fontWeight:700}}>{v}</span>
          </div>
        ))}
      </div>
      <div style={{background:"linear-gradient(135deg,#0d3300,#6DC21E)", borderRadius:16, padding:"14px 20px", width:"100%", color:"#fff", textAlign:"center", marginBottom:14}}>
        <div style={{fontSize:12, opacity:.8, marginBottom:2, fontWeight:500}}>Nouveau solde</div>
        <div style={{fontSize:30, fontWeight:800}}>{nvSolde.toLocaleString("fr-FR")} <span style={{fontSize:14, opacity:.8}}>FCFA</span></div>
      </div>
      <div onClick={() => {setCopied(true); setTimeout(() => setCopied(false), 2000);}} style={{background:"#F7FAF2", borderRadius:10, padding:"10px 15px", fontFamily:"monospace", fontSize:13, color:"#4A5E38", marginBottom:copied?4:14, display:"flex", alignItems:"center", gap:8, cursor:"pointer", width:"100%"}}>
        <span style={{flex:1}}>Réf : {ref}</span><span>📋</span>
      </div>
      {copied && <div style={{color:G, fontSize:12, fontWeight:700, marginBottom:10}}>Copié !</div>}
      <div style={{width:"100%", display:"flex", flexDirection:"column", gap:10}}>
        <button className="btn-press" onClick={onBack} style={{width:"100%", padding:15, border:"none", borderRadius:14, background:"linear-gradient(135deg,"+G+","+GD+")", color:"#fff", fontSize:15, fontWeight:800, cursor:"pointer"}}>Retour à l'accueil</button>
        <button className="btn-press" style={{width:"100%", padding:13, border:"1.5px solid #D8EBC0", borderRadius:14, background:"#fff", color:"#4A5E38", fontSize:14, fontWeight:700, cursor:"pointer"}}>Partager le reçu</button>
      </div>
    </div>
  );
}
...
// ── HISTORIQUE ────────────────────────────────────────────────────────────────
function Histo({histo}) {
  const [f, setF] = useState("Tout");
  const list = histo.filter(t => f==="Tout" || (f==="Envoyé" ? t.ok : !t.ok));
  return (
    <div className="fadeIn" style={{flex:1, overflowY:"auto", paddingBottom:85}}>
      <style>{css}</style>
      <div style={{padding:"14px 16px 10px", background:"#fff", borderBottom:"1px solid #D8EBC0", position:"sticky", top:0, zIndex:10}}>
        <div style={{fontSize:19, fontWeight:800}}>Historique</div>
      </div>
      <div style={{display:"flex", gap:8, padding:"12px 14px 0", overflowX:"auto"}}>
        {["Tout","Envoyé","En attente"].map(fil => (
          <button key={fil} className="btn-press" onClick={() => setF(fil)} style={{flexShrink:0, padding:"7px 14px", borderRadius:20, border:"1.5px solid "+(f===fil?G:"#D8EBC0"), background:f===fil?G:"#fff", color:f===fil?"#fff":"#8FAD72", fontSize:12, fontWeight:700, cursor:"pointer"}}>{fil}</button>
        ))}
      </div>
      <div style={{padding:"12px 14px 0"}}>
        {list.map(t => <TxRow key={t.id} t={t}/>)}
        {list.length === 0 && <div style={{textAlign:"center", padding:"48px 0", color:"#8FAD72"}}><div style={{fontSize:36, marginBottom:10}}>📭</div><div style={{fontWeight:700}}>Aucune transaction</div></div>}
      </div>
    </div>
  );
}
...
// ── PROFIL ────────────────────────────────────────────────────────────────────
function Profil({user, histo, onLogout}) {
  const total = histo.filter(t => t.ok).reduce((s,t) => s+t.mnt, 0);
  const menus = [
    {ic:"🛡️", lbl:"Vérification KYC", sub:"Non vérifié — Limite 327 500 FCFA"},
    {ic:"👥", lbl:"Mes bénéficiaires", sub:CONTACTS.length+" contacts"},
    {ic:"🔔", lbl:"Notifications", sub:"Activées"},
    {ic:"💬", lbl:"Aide et Support", sub:"Chat 24h/24"},
    {ic:"🔒", lbl:"Sécurité & PIN", sub:"Modifier mon PIN"},
    {ic:"❓", lbl:"À propos", sub:"Cash Money v2.0"},
  ];
  return (
    <div className="fadeIn" style={{flex:1, overflowY:"auto", paddingBottom:85}}>
      <style>{css}</style>
      <div style={{background:"linear-gradient(135deg,#0d3300,"+G+")", padding:"28px 18px 48px", color:"#fff", textAlign:"center"}}>
        <Logo h={26}/>
        <div style={{width:76, height:76, borderRadius:"50%", background:"rgba(255,255,255,.2)", border:"3px solid rgba(255,255,255,.4)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:32, margin:"14px auto 10px"}}>👤</div>
        <div style={{fontSize:21, fontWeight:800}}>{user.nom}</div>
        <div style={{color:"rgba(255,255,255,.75)", fontSize:13, marginTop:3, fontWeight:500}}>{user.tel}</div>
        <div style={{display:"inline-block", marginTop:10, padding:"4px 12px", borderRadius:20, background:"rgba(255,255,255,.2)", fontSize:11, fontWeight:700}}>✓ Compte actif</div>
      </div>
      <div style={{background:"#fff", borderRadius:18, margin:"-24px 14px 14px", overflow:"hidden", boxShadow:"0 4px 20px rgba(0,0,0,.08)"}}>
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr", textAlign:"center", padding:"16px 0"}}>
          {[{l:"Envoyé",v:total.toLocaleString("fr-FR")+"F"},{l:"Transferts",v:histo.length},{l:"Pays",v:new Set(histo.map(t=>t.flag)).size}].map(({l,v}) => (
            <div key={l}>
              <div style={{fontSize:16, fontWeight:800, color:G}}>{v}</div>
              <div style={{fontSize:11, color:"#8FAD72", marginTop:2, fontWeight:500}}>{l}</div>
            </div>
          ))}
        </div>
      </div>
      <div style={{background:"#fff", borderRadius:18, margin:"0 14px 14px", overflow:"hidden"}}>
        {menus.map((m,i) => (
          <div key={m.lbl} style={{display:"flex", alignItems:"center", gap:12, padding:"14px 16px", borderBottom:i<menus.length-1?"1px solid #D8EBC0":"none", cursor:"pointer"}}>
            <div style={{width:38, height:38, borderRadius:12, background:"#eef8dc", display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, flexShrink:0}}>{m.ic}</div>
            <div style={{flex:1}}>
              <div style={{fontSize:14, fontWeight:700}}>{m.lbl}</div>
              <div style={{fontSize:11, color:"#8FAD72", marginTop:1, fontWeight:500}}>{m.sub}</div>
            </div>
            <ChevR/>
          </div>
        ))}
      ...
      <div style={{padding:"0 14px 14px"}}>
        <button className="btn-press" onClick={onLogout} style={{width:"100%", padding:13, border:"1.5px solid #e53e3e", borderRadius:14, background:"#fff", color:"#e53e3e", fontSize:14, fontWeight:800, cursor:"pointer"}}>
          Déconnexion
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  const [solde, setSolde]     = useState(820000);
  const [histo, setHisto]     = useState(HISTO0);
  const [toast, setToast]     = useState(null);
  const [notifs, setNotifs]   = useState(NOTIFS);
  const [showNotif, setNotif] = useState(false);
  const notifCount = notifs.filter(n => !n.lu).length;
  const showToast = (msg) => {setToast(msg); setTimeout(() => setToast(null), 3000);};
  const handleSend = (c=null) => {setPre(c); setSend(true);};
  const handleDone = (data) => {
    const nv = Math.max(0, solde - data.total);
    setSolde(nv);
    setHisto(h => [{id:Date.now(), dest:data.contact.nom, flag:data.pays.flag, mnt:data.amt, recu:data.recu, dev:data.pays.devise, op:data.op, date:new Date().toLocaleDateString("fr-FR"), ok:true}, ...h]);
    setSend(false); setSucc({...data, nvSolde:nv});
  };
  const handleBack = () => {setSucc(null); showToast("✅ Transfert confirmé !"); setTab("accueil");};
  const markAllRead = () => setNotifs(n => n.map(x => ({...x, lu:true})));

  const shell = {maxWidth:430, margin:"0 auto", minHeight:"100vh", background:"#F3F7EE", fontFamily:"'Plus Jakarta Sans', system-ui, sans-serif", color:"#1A2E0A", display:"flex", flexDirection:"column", position:"relative"};

  if (splash) return <div style={shell}><style>{css}</style><Splash onDone={() => setSplash(false)}/></div>;
  if (!user)  return <div style={shell}><style>{css}</style><Auth onAuth={setUser}/></div>;
  if (success) return <div style={shell}><style>{css}</style><Success data={success} nvSolde={success.nvSolde} onBack={handleBack}/></div>;
  if (sending) return <div style={shell}><style>{css}</style><Send pre={pre} onBack={() => setSend(false)} onDone={handleDone}/></div>;
  if (showNotif) return <div style={shell}><style>{css}</style><Notifications notifs={notifs} onBack={() => setNotif(false)} onMarkAll={markAllRead}/></div>;

  const navItems = [{id:"accueil",ic:"🏠",lbl:"Accueil"},{id:"envoyer",ic:"✈️",lbl:"Envoyer"},{id:"historique",ic:"🕐",lbl:"Historique"},{id:"profil",ic:"👤",lbl:"Profil"}];

  return (
    <div style={shell}>
      <style>{css}</style>
      {tab === "accueil"    && <Home   solde={solde} histo={histo} onSend={handleSend} onHisto={() => setTab("historique")} user={user} onNotif={() => setNotif(true)} notifCount={notifCount}/>}
      {tab === "historique" && <Histo  histo={histo}/>}
      {tab === "profil"     && <Profil user={user} histo={histo} onLogout={() => setUser(null)}/>}
      {toast && (
        <div style={{position:"fixed", bottom:90, left:"50%", transform:"translateX(-50%)", background:"#0D1B2A", color:"#fff", padding:"10px 20px", borderRadius:14, fontSize:13, fontWeight:700, zIndex:100, whiteSpace:"nowrap", boxShadow:"0 4px 16px rgba(0,0,0,.3)"}}>
          {toast}
        </div>
      )}
      <nav style={{position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:430, background:"#fff", borderTop:"1px solid #D8EBC0", display:"flex", padding:"8px 0 18px", zIndex:20}}>
        {navItems.map(n => (
          <button key={n.id} className="btn-press" onClick={() => n.id==="envoyer" ? handleSend() : setTab(n.id)} style={{flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3, cursor:"pointer", border:"none", background:"transparent", padding:"3px 0"}}>
            <div style={{width:20, height:3, borderRadius:2, background:tab===n.id?G:"transparent", marginBottom:-1, transition:"background .2s"}}/>
            <span style={{fontSize:20}}>{n.ic}</span>
            <span style={{fontSize:10, fontWeight:700, color:tab===n.id?G:"#8FAD72"}}>{n.lbl}</span>
          </button>
        ))}
      </nav>
    </div>
